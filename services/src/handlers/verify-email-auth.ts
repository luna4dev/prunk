...
export class VerifyEmailAuthContext
  ...
  implements IJWTContext
{
  ...
  constructor(action: string, environment: any) {
    ...
    this.EMAIL_AUTH_TOKEN_EXPIRATION_TIME =
      parseInt(environment.EMAIL_AUTH_TOKEN_EXPIRATION_TIME) || 1000 * 60 * 30; // 10 minutes

    this.JWT_EXPIRATION_TIME =
      parseInt(environment.JWT_EXPIRATION_TIME) || 1000 * 60 * 60 * 24 * 30; // 30 days
...
export const handler = async (
  ...
  try {
    ...
    const { userId } = await verifyEmailAuth(
      email,
      token,
      EMAIL_AUTH_TOKEN_EXPIRATION_TIME,
      context
    );

    // create JWT
    const jwtToken = createJWT({ userId }, context, {
    ...
    if (error instanceof PrunkError && error.statusCode !== 500) {
      return handleErrorResponse(
        new PrunkError('Malformed email verification', 400)
      );
    }
